...
(function(b,c){var d=wp.media.view.l10n="undefined"===typeof _wpMediaViewsL10n?{}:_wpMediaViewsL10n,a=-1<navigator.userAgent.indexOf("Chrome"),e=!a&&-1<navigator.userAgent.indexOf("Safari");_.extend(d,cheese_l10n);c.cheese={supports:{paste:!e&&cheese_l10n.enable_pasteboard&&("paste"in document||"onpaste"in document||"object"===typeof window.onpaste),webcam_recording:cheese_l10n.enable_snapshot&&b.recorder.supported&&(!a||a&&"https:"===location.protocol)},view:{}}})(jQuery,wp.media);
...
(function(d,l,g){var e=wp.media.view.Button,h=wp.media.view.Modal,c=wp.media.view.l10n="undefined"===typeof _wpMediaViewsL10n?{}:_wpMediaViewsL10n,c=_.extend(c,cheese_l10n);wp.media.cheese.view.NameInput=wp.media.View.extend({tagName:"label",className:"setting",_input:null,initialize:function(){_.defaults(this.options,{defaultValue:"untitled",title:c.title});this._input=d('<input class="alignment" type="text" data-setting="title" />');this.$el.append("<span>"+this.options.title+"</span>").append(this._input)},
...
this.$el.removeClass("disabled");return a}});wp.media.cheese.view.DataSourceImageUploader=wp.media.View.extend({tagName:"div",className:"data-source-image",controller:null,image:null,discardBtn:null,nameInput:null,uploadBtn:null,uploader:null,_model:{},initialize:function(){_.defaults(this.options,{defaultFileName:c.image});var a=this,b=new wp.media.View({tagName:"div",className:"instruments",controller:this.controller});this.discardBtn=new wp.media.cheese.view.ActionButton({className:"image-discard",
style:"secondary",text:c.try_again,dashicon:"arrow-left",controller:this.controller,click:function(){a.discardImage.apply(a,arguments)}});this.nameInput=new wp.media.cheese.view.NameInput({defaultValue:this.options.defaultFileName});this.uploadBtn=new wp.media.cheese.view.ActionButton({className:"image-upload",style:"primary",text:c.upload,controller:this.controller,click:function(){a.uploadImage.apply(a,arguments)}});this.views.add(b);b.views.add(this.discardBtn);b.views.add(this.nameInput);b.views.add(this.uploadBtn);
this.$imageContainer=!1},setImageData:function(a){var b=this.$el.find(".image-container").html("").get(0),c=this;this.image&&this.image.destroy();this.image=new g.Image;this.image.onload=function(){var a=c.controller.uploader.uploader.uploader.getOption("resize"),d=Math.max(a.width/this.width,a.height/this.height);a&&1>d&&this.downsize(this.width*d,this.height*d);this.embed(b)};this.image.bind("Resize",function(a){this.embed(b)});this.image.load(a);this.$imageContainer&&this.$imageContainer.append(this.image);
this.disabled(!1);return this},render:function(){this.$imageContainer=d('<span class="image-container" />');this.$el.prepend(this.$imageContainer)},discardImage:function(){this.controller.trigger("action:discard:dataimage",this)},uploadImage:function(){this.bindEvents();var a=this.nameInput.val()+".png",b=this.image.getAsBlob("image/png");b.detach(b.getSource());b.name=a;b.type="image/png";this.controller.uploader.uploader.uploader.addFile(b,a);this.disabled(!0);this.controller.trigger("action:upload:dataimage",
this)},show:function(){this.$el.show();return this},hide:function(){this.$el.hide();return this},disabled:function(a){this.uploadBtn.model.disabled=this.discardBtn.model.disabled=a;this.uploadBtn.render();this.discardBtn.render()},_uploadSuccessHandler:function(){this.controller.trigger("action:uploaded:dataimage");this.disabled(!1)},_uploadErrorHandler:function(){this.controller.trigger("error:uploaded:dataimage");this.disabled(!1)},bindEvents:function(){this.controller.uploader.uploader.uploader.bind("FileUploaded",
this._uploadSuccessHandler,this);this.controller.uploader.uploader.uploader.bind("Error",this._uploadErrorHandler,this)}});wp.media.cheese.view.WebcamRecorder=wp.media.View.extend({tagName:"div",className:"webcam-recorder",controller:null,action:"record",_webcam:null,_recorder:null,_instruments:null,initialize:function(){_.defaults(this.options,{});var a=this;this._recorder=d('<div class="recorder"></div>').appendTo(this.$el).on("click",".recorder-record",function(b){b.preventDefault();a.controller.trigger("action:create:dataimage",
a,a._webcam.snapshot());a.stop();return!1});this._instruments=d('<div class="instruments"><a href="#" class="button recorder-record button-primary button-action"><span class="dashicons dashicons-video-alt2"></span>'+c.record+"</a></div>").appendTo(this._recorder);this.$el.on("click",".error-try-again",function(){a.start();a.$el.find(".error").remove()});if(!this._webcam){var b={camera:{mandatory:{minWidth:640,minHeight:480}},microphone:!1,flash:{swf_url:c.swf_url}};this._webcam=d(this._recorder).recorder(b);
this._webcam.on("recorder:state:ready",function(a){});this._webcam.on("recorder:state:waiting",function(b){a._instruments.hide()});this._webcam.on("recorder:state:started",function(b){a._instruments.show()});this._webcam.on("recorder:state:error",function(b){a.$el.append('<div class="error recorder-inline-content"><h3>'+c.an_error_occured+'</h3><p><a class="error-try-again" href="#">'+c.try_again+"</a></p></div>");a._instruments.hide()});this._webcam.on("recorder:state:permissionerror",function(b,
d,e){a.$el.append('<div class="error recorder-inline-content"><h3>'+(e.message||c.please_allow_camera_message)+'</h3><p><a class="error-try-again" href="#">'+c.try_again+"</a></p></div>");a._instruments.hide()});this._webcam.on("recorder:state:stopped",function(b){a._instruments.hide()})}this._instruments.hide()},get_state:function(){return this._webcam.state},start:function(){this._recorder.is(":visible")||this._recorder.show();this._webcam.start();return this},stop:function(){this._webcam.stop();
this._recorder.is(":visible")&&this._recorder.hide();return this},show:function(){this.$el.show();return this},hide:function(){this.$el.hide();return this}});wp.media.cheese.view.Pasteboard=wp.media.View.extend({tagName:"div",className:"pasteboard",controller:null,action:"paste",_content:null,_pasteboard:null,initialize:function(){_.defaults(this.options,{});this._content=d('<div class="pasteboard-inline-content"><h3>'+c.paste_instructions+"</h3></div>").appendTo(this.$el);this._pasteboard=d('<div id="pasteboard-injector"></div>').appendTo(this._content)},
start:function(){var a=this;this._pasteboard.imagepastebox({messages:{no_image_pasted:c.paste_error_no_image,no_processible_image_pasted:c.paste_error_webkit_fake_image}}).on("pasteimage",function(b,c){a._onPaste(c)}).focus();this.listenTo(this.pasteboard,"pasteimage",this._onPaste);return this},stop:function(){this._pasteboard.imagepastebox("off").off("pasteimage");return this},show:function(){this.$el.show();return this},hide:function(){this.$el.hide();return this},_onPaste:function(a){this.controller.trigger("action:create:dataimage",
this,a)}});var k=0;wp.media.cheese.view.DataSourceImageGrabber=wp.media.View.extend({tagName:"div",className:"image-grabber",_grabber:null,_uploader:null,initialize:function(){this.id=k++;_.defaults(this.options,{grabber:wp.media.cheese.view.WebcamRecorder});var a=this.options.grabber==wp.media.cheese.view.WebcamRecorder?c.snapshot:this.options.grabber==wp.media.cheese.view.Pasteboard?c.pasted:c.image,b=this.options.grabber==wp.media.cheese.view.WebcamRecorder?c.take_snapshot:this.options.grabber==
wp.media.cheese.view.Pasteboard?c.copy_paste:c.image,d=new wp.media.View({className:"image-grabber-content",tagName:"div",controller:this.controller}),e=new wp.media.View({className:"media-frame-title",tagName:"div"}),f=new wp.media.View({tagName:"h1"});this._grabber=new this.options.grabber({controller:this.controller});this._uploader=new wp.media.cheese.view.DataSourceImageUploader({controller:this.controller,defaultFileName:a});d.views.add(this._grabber);d.views.add(this._uploader);f.$el.text(b);
e.views.add(f);this.views.add(e);this.views.add(d)},imageCreated:function(a,b){this._grabber.stop().hide();this._uploader.show().setImageData(b)},startGrabbing:function(){this._uploader.hide();this._grabber.show().start();return this},getAction:function(){return this._grabber.action},activate:function(){this.controller.on("action:create:dataimage",this.imageCreated,this);this.controller.on("action:discard:dataimage",this.startGrabbing,this);return this},dismiss:function(){this._grabber.stop();this.controller.off("action:create:dataimage",
this.imageCreated);this.controller.off("action:discard:dataimage",this.startGrabbing);return this}});wp.media.cheese.view.GrabberButton=e.extend({className:"grabber-button",_grabber:null,_modal:null,initialize:function(a){e.prototype.initialize.apply(this,arguments);var b;_.defaults(this.options,{grabber:null,title:"Image Grabber"});this._grabber=new wp.media.cheese.view.DataSourceImageGrabber({controller:this.controller,grabber:this.options.grabber});this._modal=new h({controller:d(),title:this.options.title});
this._modal.content(this._grabber);b=this._grabber.getAction();this.controller.on(b+":activate "+b+":deactivate",this.toggleModeHandler,this);this.controller.on(b+":action:done",this.back,this);this.controller.on("action:uploaded:dataimage",this.uploadDone,this);this.controller.on("error:uploaded:dataimage",this.uploadDone,this);this._modal.on("close",this.back,this)},uploadDone:function(){this._modal.close()},back:function(){this._grabber.dismiss();this.controller.deactivateMode(this._grabber.getAction()).activateMode("edit")},
click:function(){e.prototype.click.apply(this,arguments);this.controller.isModeActive(this._grabber.getAction())||(this.controller.deactivateMode("edit").activateMode(this._grabber.getAction()),this._modal.open(),this._grabber.activate().startGrabbing())}})})(jQuery,window,mOxie);

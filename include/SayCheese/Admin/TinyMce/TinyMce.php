...
<?php echo file_get_contents( $this->core->get_asset_path( 'js/admin/mce/'.$this->module_name.'-plugin'.$suffix.'.js' ) ); ?>;
<?php echo $this->module_name ?>PluginCallback( editor );
	...
	public function add_plugin( $plugins_array ) {
		$suffix = defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ? '-with-sourcemap' : '';
		$plugins_array[ $this->module_name ] = $this->core->get_asset_url( 'js/admin/mce/'.$this->module_name.'-plugin'.$suffix.'.js' );
	...
	public function enqueue_toolbar_css() {
		...
		$asset_url = $this->core->get_asset_url( 'css/admin/mce/cheese-toolbar.css' );
	...
	public function mce_css( $styles ) {
		$mce_css = $this->core->get_asset_url( 'css/admin/mce/cheese-editor.css' );
		$styles .= ','. $mce_css;
		return $styles;
	}
